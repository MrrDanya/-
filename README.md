...
<body>
    ...
    <div id="crypto-section" class="section">
        ...
        <div class="crypto-container">
            ...
            <div class="crypto-card animate__animated animate__fadeIn">
                <div class="crypto-header">
                    <div class="crypto-icon">🅃</div>
                    <div class="crypto-name">Rich TON (RTN)</div>
                </div>
                <div class="crypto-price" id="rtn-price">₽0</div>
                <div class="crypto-change" id="rtn-change">0%</div>
                <div class="crypto-stats">
                    <p>Ваш баланс: <span id="rtn-balance">0</span> RTN</p>
                    <p>Стоимость: <span id="rtn-value">0</span> ₽</p>
                </div>
                <div class="crypto-buttons">
                    <button class="buy-button" onclick="showCryptoModal('rtn', 'buy')">Купить</button>
                    <button class="sell-button" onclick="showCryptoModal('rtn', 'sell')">Продать</button>
                </div>
            </div>
        </div>
    ...
    <div id="crypto-modal" class="modal" style="display: none;">
        <div class="modal-content">
            ...
            <input type="number" id="crypto-modal-amount" placeholder="Введите сумму в ₽" class="crypto-input">
            <p>Получите: <span id="crypto-modal-sum">0</span></p>
    ...
    <script>
        ...
        const GLOBAL_CRYPTO = {
            ...
            rtn: { price: 10000000, history: [], lastChange: 0 } // Изменено с 1к на 10м
        };
        ...
        function updateCryptoPrices() {
            ...
            document.getElementById('rtn-price').textContent = `${GLOBAL_CRYPTO.rtn.price.toLocaleString('ru-RU')} ₽`;
            
            // Обновляем изменение цены
            updateCryptoChange('btc');
            updateCryptoChange('eth');
            updateCryptoChange('rtn');
        ...
        function updateCryptoBalances() {
            ...
            // RTN
            document.getElementById('rtn-balance').textContent = crypto.rtn.balance.toFixed(2);
            document.getElementById('rtn-value').textContent = (crypto.rtn.balance * GLOBAL_CRYPTO.rtn.price).toLocaleString('ru-RU');
            
            // Общая стоимость портфеля
            const totalValue = (crypto.btc.balance * GLOBAL_CRYPTO.btc.price) + 
                              (crypto.eth.balance * GLOBAL_CRYPTO.eth.price) + 
                              (crypto.rtn.balance * GLOBAL_CRYPTO.rtn.price);
        ...
        function updateCharacterCrypto() {
            ...
            if (crypto.btc.balance > 0 || crypto.eth.balance > 0 || crypto.rtn.balance > 0) {
                ...
                if (crypto.rtn.balance > 0) {
                    const rtnItem = document.createElement('div');
                    rtnItem.className = 'portfolio-item';
                    rtnItem.innerHTML = `
                        <span>Rich TON (RTN):</span>
                        <span>${crypto.rtn.balance.toFixed(2)} (${(crypto.rtn.balance * GLOBAL_CRYPTO.rtn.price).toLocaleString('ru-RU')} ₽)</span>
                    `;
                    cryptoList.appendChild(rtnItem);
                }
            } else {
        ...
        function updateCryptoRandomly() {
            // BTC: изменение от -15% до +20%
            const btcChange = (Math.random() * 35 - 15) / 100;
            ...
            // ETH: изменение от -15% до +20%
            const ethChange = (Math.random() * 35 - 15) / 100;
            ...
            // RTN: изменение от -15% до +20%
            const rtnChange = (Math.random() * 35 - 15) / 100;
            GLOBAL_CRYPTO.rtn.price = Math.max(1000000, GLOBAL_CRYPTO.rtn.price * (1 + rtnChange)); // Минимум 1м
            GLOBAL_CRYPTO.rtn.history.push(GLOBAL_CRYPTO.rtn.price);
            if (GLOBAL_CRYPTO.rtn.history.length > 10) GLOBAL_CRYPTO.rtn.history.shift();
            GLOBAL_CRYPTO.rtn.lastChange = rtnChange * 100;
        ...
        function showCryptoModal(type, action) {
            ...
            let cryptoName = '';
            switch(type) {
                case 'btc': cryptoName = 'Bitcoin (BTC)'; break;
                case 'eth': cryptoName = 'Ethereum (ETH)'; break;
                case 'rtn': cryptoName = 'Rich TON (RTN)'; break;
            }
            ...
            if (action === 'buy') {
                ...
                available.textContent = `${(crypto[type].balance * cryptoData.price).toLocaleString('ru-RU')} ₽`;
                document.getElementById('crypto-modal-sum').textContent = crypto[type].balance.toFixed(type === 'btc' ? 8 : type === 'eth' ? 6 : 2);
            }
            ...
            amountInput.oninput = function() {
                ...
                if (action === 'buy') {
                    ...
                    document.getElementById('crypto-modal-sum').textContent = cryptoAmount.toFixed(type === 'btc' ? 8 : type === 'eth' ? 6 : 2);
                } else {
                    const cryptoAmount = amount / cryptoData.price;
                    if (cryptoAmount > crypto[type].balance) {
                        document.getElementById('crypto-modal-sum').textContent = crypto[type].balance.toFixed(type === 'btc' ? 8 : type === 'eth' ? 6 : 2);
                    } else {
                        document.getElementById('crypto-modal-sum').textContent = cryptoAmount.toFixed(type === 'btc' ? 8 : type === 'eth' ? 6 : 2);
                    }
        ...
        function executeCryptoTransaction() {
            ...
            if (action === 'buy') {
                ...
                const cryptoAmount = amount / cryptoData.price;
                balance -= amount;
                crypto[type].balance += cryptoAmount;
            } else {
                const maxAmount = crypto[type].balance * cryptoData.price;
                if (amount > maxAmount) {
            ...
            const notification = document.createElement('div');
            notification.className = 'game-result win animate__animated animate__bounceIn';
            notification.textContent = `${action === 'buy' ? 'Покупка' : 'Продажа'} успешна: ${parseFloat(document.getElementById('crypto-modal-sum').textContent)} ${type.toUpperCase()} за ${amount.toLocaleString('ru-RU')} ₽`;
            notification.style.position = 'fixed';
            notification.style.bottom = '20px';
            notification.style.right = '20px';
            notification.style.zIndex = '1000';
